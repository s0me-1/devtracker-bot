...
class Guilds(commands.Cog):
    ...
    async def on_guild_join(self, guild : disnake.Guild):
        ...
        await dt_channel.send(f'`{guild.name} [{guild.id}]` joined. (Approx `{guild.approximate_member_count}` members)')
    ...
    async def on_guild_remove(self, guild : disnake.Guild):
        ...
        dt_channel = self.bot.get_channel(985250371981172757)
        await dt_channel.send(f'{guild.name} [{guild.id}] removed. (Approx `{guild.approximate_member_count}` members)')
